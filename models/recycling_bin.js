...
export class RecyclingBinModel {
  ...
  static deletePermanently(roId, resp) {
    const sqlQuery1 = 'DELETE FROM recycling_bin WHERE recycling_bin.ro_id = ?';
    const sqlQuery2 = 'DELETE * FROM authors WHERE authors.ro_id = ?';
    return new Promise(() => {
      //remove ro from the bin
      connection.query(sqlQuery1, [roId], (err) => {
        if (err) {
          throw (err);
        }
      });
      //removing authros on from the research
